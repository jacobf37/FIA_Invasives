...
inv_sp <- inv_tpa %>% left_join(inv_plot, by = c('PLT_CN' = 'CN')) %>% 
  select(LAT, LON, SPCD, pltID, INVYR, STATECD) %>% 
...
saveRDS(inv_sp, './Data/inv_sp.rds')
```
<br>
Map it.
...
```{r}
inv_cp <- inv_sp %>% mutate(DISPLYR = INVYR)
out_tb <- tibble()
for (i in unique(inv_sp$SPCD)){
  for (j in unique(inv_sp %>% filter(SPCD == i) %>% select(INVYR) %>% pull())){
    out_tb <- out_tb %>% bind_rows(
      inv_cp %>% filter(SPCD == i & INVYR < j) %>% mutate(DISPLYR = j))
  }
}

inv_cp <- inv_cp %>% bind_rows(out_tb)
saveRDS(inv_cp, './Data/display_tb.rds')
table(inv_sp %>% filter(SPCD == 'Chinese tallowtree') %>% select(STATECD, INVYR))
```

<br>
```{r FIA invasives estimate}
inv_tb <- invasive(db, byPlot = T) 
inv_subp <- inv_tb %>% left_join(sub_db$PLOT %>%
  select(CN, PREV_PLT_CN, INVYR, LAT, LON, STATECD, COUNTYCD, PLOT, ECOSUBCD), by = c('PLT_CN' = 'CN')) %>% 
  #select(LAT, LON, SPCD, pltID, INVYR, STATECD) %>% 
  filter(!(is.na(LAT)), !(is.na(LON)), SYMBOL %in% c('AIAL', 'PATO2', 'MEAZ', 'TRSE6')) %>% 
  mutate(SYMBOL = as.factor(SYMBOL)) %>% 
  mutate(SYMBOL = recode_factor(SYMBOL, AIAL = 'ailanthus', PATO2 = 'empress-tree', MEAZ = 'chinaberry', TRSE6 = 'Chinese tallowtree'))
```

```{r map-invasive-subplots, message=FALSE}
states_lyr <- map_data("state")
...
inv_subp_map <- ggplot() + 
  geom_polygon(data=states_lyr, aes(x=long, y=lat, group=group),
                color="black", fill="white" ) +
  labs(x = 'Longitude', y = 'Latitude') +
  geom_point(data=inv_subp, aes(x=LON, y=LAT, fill = SYMBOL),
             colour="black",pch=21) +
  coord_cartesian(xlim=c(-105, -70), ylim = c(25, 45)) +
  theme_classic() +
  theme(legend.position = 'none') +
